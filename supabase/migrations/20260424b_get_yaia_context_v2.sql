-- get_yaia_context v2: snapshot completo do bebe pra alimentar a yaIA.
-- Antes a funcao trazia so pending/active de vacinas e 10 marcos. Agora
-- devolve todas as informacoes relevantes (aplicadas + pendentes, marcos
-- completos por categoria, medidas recentes, agregados dos ultimos 7 dias,
-- medicamentos ativos e inativos recentes, humor em saltos).
...
BEGIN
  ...
  SELECT json_build_object(
    'total_sleep_minutes', COALESCE(SUM(duration) FILTER (WHERE event_id = 'sleep'), 0),
    'sleep_sessions', COUNT(*) FILTER (WHERE event_id = 'sleep'),
    'total_feed_ml', COALESCE(SUM(ml) FILTER (WHERE event_id IN ('bottle', 'formula')), 0),
    'breast_sessions', COUNT(*) FILTER (WHERE event_id LIKE 'breast%'),
    'bottle_sessions', COUNT(*) FILTER (WHERE event_id IN ('bottle', 'formula')),
    'diaper_pee', COUNT(*) FILTER (WHERE event_id = 'diaper_pee'),
    'diaper_poop', COUNT(*) FILTER (WHERE event_id = 'diaper_poop'),
    'diaper_both', COUNT(*) FILTER (WHERE event_id = 'diaper_both'),
