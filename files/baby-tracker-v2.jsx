...
function getNextProjection(logs, category, intervals) {
  const relevant = logs.filter(l => {
    const ev = DEFAULT_EVENTS.find(e => e.id === l.eventId);
...
function ModalTitle({ children }) {
  return <div style={{ fontWeight: 700, fontSize: 17, marginBottom: 18 }}>{children}</div>;
}

const btn = (extra = {}) => ({
  padding: "11px 0", borderRadius: 12, cursor: "pointer",
  fontWeight: 700, fontSize: 14, border: "none", ...extra,
});
...
function EditLogModal({ log, onSave, onDelete, onClose }) {
  ...
  function handleSave() {
    ...
    const newTs = new Date(y, mo - 1, d, h, m).getTime();
    onSave({ ...log, timestamp: newTs, ml: ml ? parseInt(ml) : undefined });
  }

  return (
    <Modal onClose={onClose}>
      <ModalTitle>{ev?.icon} Editar — {ev?.label}</ModalTitle>
      ...
      {ev?.hasAmount && (
        <div style={{ marginBottom: 14 }}>
          ...
          <input type="number" value={ml} onChange={e => setMl(e.target.value)}
            placeholder="ex: 60" style={{ ...inputStyle, width: "100%" }} />
        </div>
      )}
        ...
        {!confirmDelete ? (
          <>
            <button onClick={() => setConfirmDelete(true)}
              style={{ ...btn(), flex: 1, background: "rgba(239,68,68,0.15)", color: "#f87171", border: "1.5px solid rgba(239,68,68,0.3)" }}>
              Excluir
            </button>
            <button onClick={onClose}
              style={{ ...btn(), flex: 1, background: "rgba(255,255,255,0.07)", color: "#f0eeff", border: "1.5px solid rgba(255,255,255,0.1)" }}>
              Cancelar
            </button>
            <button onClick={handleSave}
              style={{ ...btn(), flex: 2, background: "linear-gradient(135deg,#7c3aed,#a855f7)", color: "#fff" }}>
              Salvar
            </button>
          </>
        ) : (
          <>
            ...
            <button onClick={() => onDelete(log.id)}
              style={{ ...btn(), flex: 2, background: "linear-gradient(135deg,#dc2626,#ef4444)", color: "#fff" }}>
              Confirmar exclusão
            </button>
          </>
        )}
...
function IntervalsModal({ intervals, onSave, onClose }) {
  const [vals, setVals] = useState(() =>
    Object.fromEntries(Object.entries(intervals).map(([k, v]) => [k, { ...v }]))
  );
  ...
  return (
    <Modal onClose={onClose}>
      <ModalTitle>⏱ Intervalos esperados</ModalTitle>
      <div style={{ fontSize: 12, color: "#94a3b8", marginBottom: 16 }}>
        Ajuste conforme a rotina do seu bebê foi se formando.
      </div>
        ...
        <div key={cat} style={{ marginBottom: 18 }}>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            <div style={{ flex: 1 }}>
              ...
              <input type="number" value={v.warn} onChange={e => set(cat, "warn", e.target.value)}
                style={{ ...inputStyle, width: "100%" }} />
            </div>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 10, color: "#64748b", marginBottom: 4 }}>LIMITE (min)</div>
              <input type="number" value={v.minutes} onChange={e => set(cat, "minutes", e.target.value)}
                style={{ ...inputStyle, width: "100%" }} />
            </div>
          </div>
        </div>
...
function BottleModal({ onConfirm, onClose }) {
  ...
  return (
    <Modal onClose={onClose}>
      <ModalTitle>🍼 Quanto mamou?</ModalTitle>
      ...
      <input type="number" value={amount} onChange={e => setAmount(e.target.value)}
        style={{ ...inputStyle, width: "100%", textAlign: "center", marginBottom: 16 }}
      ...
      <div style={{ display: "flex", gap: 10 }}>
        ...
        <button onClick={() => onConfirm(parseInt(amount) || 0)}
          style={{ ...btn(), flex: 2, background: "linear-gradient(135deg,#7c3aed,#a855f7)", color: "#fff" }}>
          Registrar
        </button>
      </div>
...
export default function BabyTracker() {
  const [logs, setLogs] = useState(() => {
    try { return JSON.parse(localStorage.getItem("babylogs_v2") || "[]"); } catch { return []; }
  });
  const [intervals, setIntervals] = useState(() => {
    try { return JSON.parse(localStorage.getItem("babyintervals") || "null") || DEFAULT_INTERVALS; } catch { return DEFAULT_INTERVALS; }
  });
  ...
  const [intervalsModal, setIntervalsModal] = useState(false);
  ...
  useEffect(() => {
    localStorage.setItem("babylogs_v2", JSON.stringify(logs.slice(-500)));
  }, [logs]);
  useEffect(() => {
    localStorage.setItem("babyintervals", JSON.stringify(intervals));
  }, [intervals]);
  ...
  function logEvent(eventId, extra = {}) {
    ...
    setLogs(prev => [...prev, { id: Date.now(), eventId, timestamp: Date.now(), ...extra }]);
    showToast(`${ev.icon} ${ev.label} registrado às ${formatTime(new Date())}`);
  }

  function handleEventTap(ev) {
    if (ev.hasAmount) setBottleModal(true);
    else logEvent(ev.id);
  }
  ...
  function handleIntervalsSave(newIntervals) {
    setIntervals(newIntervals);
    setIntervalsModal(false);
    showToast("✅ Intervalos atualizados");
  }

  const projections = ["feed", "diaper", "sleep", "bath"]
    .map(cat => getNextProjection(logs, cat, intervals))
    .filter(Boolean);

  const filteredLogs = [...logs]
    .sort((a, b) => b.timestamp - a.timestamp)
    .filter(l => activeCategory === "all" || DEFAULT_EVENTS.find(e => e.id === l.eventId)?.category === activeCategory)
    .slice(0, 30);
  ...
  return (
    <div style={{
      ...
      {/* bg blobs */}
      <div style={{ position: "fixed", top: -80, right: -80, width: 300, height: 300, background: "radial-gradient(circle, rgba(167,139,250,0.15) 0%, transparent 70%)", borderRadius: "50%", pointerEvents: "none" }} />
      <div style={{ position: "fixed", bottom: 100, left: -60, width: 250, height: 250, background: "radial-gradient(circle, rgba(249,168,212,0.12) 0%, transparent 70%)", borderRadius: "50%", pointerEvents: "none" }} />
      ...
      <div style={{ padding: "28px 20px 12px", textAlign: "center" }}>
        ...
        <div style={{ fontSize: 38, fontWeight: 800, color: "#c4b5fd", letterSpacing: "-1px", lineHeight: 1.1 }}>
          {formatTime(new Date(now))}
        </div>
      </div>

      {/* Tab nav */}
      <div style={{ display: "flex", gap: 0, margin: "0 16px 16px", background: "rgba(255,255,255,0.05)", borderRadius: 14, padding: 4 }}>
        {[{ id: "home", label: "Início" }, { id: "history", label: "Histórico" }].map(tab => (
          <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{
            flex: 1, padding: "9px 0", borderRadius: 10, border: "none", cursor: "pointer",
            background: activeTab === tab.id ? "rgba(124,58,237,0.6)" : "transparent",
            color: activeTab === tab.id ? "#fff" : "#94a3b8",
            fontWeight: activeTab === tab.id ? 700 : 500, fontSize: 14, transition: "all 0.15s",
          }}>{tab.label}</button>
        ))}
      </div>
          ...
          <div style={{ padding: "0 16px" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10 }}>
              {DEFAULT_EVENTS.map(ev => (
                <button key={ev.id} onClick={() => handleEventTap(ev)} style={{
          ...
          {projections.length > 0 && (
            <div style={{ padding: "20px 16px 0" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                <div style={{ fontSize: 11, color: "#a78bfa", letterSpacing: "0.15em", textTransform: "uppercase" }}>
                  previsão do dia
                </div>
                <button onClick={() => setIntervalsModal(true)} style={{
                  background: "rgba(167,139,250,0.15)", border: "1px solid rgba(167,139,250,0.3)",
                  borderRadius: 8, padding: "4px 12px", color: "#c4b5fd",
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {projections.map((p, i) => (
                  <div key={i} style={{
                    ...
                    <div style={{ fontWeight: 700, fontSize: 13, color: p.isOverdue ? "#f87171" : p.isWarning ? "#fbbf24" : "#a78bfa", textAlign: "right" }}>
            ...
            <div style={{ padding: "20px 16px 0" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                ...
                <button onClick={() => setActiveTab("history")} style={{
                  background: "none", border: "none", color: "#a78bfa", fontSize: 12, cursor: "pointer",
                }}>ver tudo →</button>
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {[...logs].sort((a, b) => b.timestamp - a.timestamp).slice(0, 5).map(log => {
                  const ev = DEFAULT_EVENTS.find(e => e.id === log.eventId);
                  if (!ev) return null;
                  return (
                    <LogRow key={log.id} log={log} ev={ev} now={now} onEdit={() => setEditingLog(log)} />
                  );
                })}
              </div>
            </div>
        ...
        <div style={{ padding: "0 16px" }}>
          ...
          {filteredLogs.length === 0 ? (
            <div style={{ textAlign: "center", padding: "40px 20px", color: "#475569", fontSize: 14 }}>
              ...
              <span style={{ color: "#a78bfa" }}>Toque em registrar para começar.</span>
            </div>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {filteredLogs.map(log => {
                const ev = DEFAULT_EVENTS.find(e => e.id === log.eventId);
                if (!ev) return null;
                return <LogRow key={log.id} log={log} ev={ev} now={now} onEdit={() => setEditingLog(log)} />;
              })}
            </div>
          )}
          ...
          {logs.length > 0 && (
            <div style={{ textAlign: "center", marginTop: 28 }}>
              <button onClick={() => { if (window.confirm("Limpar todo o histórico?")) setLogs([]); }} style={{
                ...
                color: "#f87171", borderRadius: 10, padding: "8px 20px", cursor: "pointer", fontSize: 12,
      ...
      {bottleModal && (
        <BottleModal
          onConfirm={ml => { logEvent("bottle", { ml }); setBottleModal(false); }}
          onClose={() => setBottleModal(false)}
        />
      )}
      {editingLog && (
        <EditLogModal
      ...
      {intervalsModal && (
        <IntervalsModal
          intervals={intervals}
          onSave={handleIntervalsSave}
          onClose={() => setIntervalsModal(false)}
        />
      )}
      ...
      {toast && (
        <div style={{
          ...
          color: "#f0eeff", zIndex: 300, whiteSpace: "nowrap",
      ...
      <style>{`* { box-sizing: border-box; } ::-webkit-scrollbar { width: 4px; height: 4px; } ::-webkit-scrollbar-thumb { background: rgba(167,139,250,0.3); border-radius: 4px; }`}</style>
    </div>
  );
}
    ...
    >
      ...
      <div style={{ textAlign: "right" }}>
        ...
        <div style={{ fontSize: 10, color: "#64748b" }}>
          {formatDate(new Date(log.timestamp))} · {formatRelative(log.timestamp - now)}
        </div>
      </div>
      <div style={{ color: "#475569", fontSize: 14, paddingLeft: 4 }}>✏️</div>
    </div>
  );
}
